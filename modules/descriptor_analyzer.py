...
class DescriptorAnalyzer:
    ...
    def __init__(self, log_file: str = None, descriptor_file: str = "lexi_descriptors.json"):
        # If no log file specified, use the most recent conversation log
        if log_file is None:
            log_dir = "lexi_conversation_logs"
            if os.path.exists(log_dir):
                # Get the most recent log file
                log_files = [f for f in os.listdir(log_dir) if f.startswith("lexi_conversation_log_") and f.endswith(".txt")]
                if log_files:
                    log_files.sort(reverse=True)  # Most recent first
                    log_file = os.path.join(log_dir, log_files[0])
                else:
                    log_file = os.path.join(log_dir, "lexi_conversation_log.txt")  # Fallback
            else:
                log_file = "lexi_chat_logs.txt"  # Legacy fallback
