...
class LexiChat:
    ...
    # Input box in right panel
    INPUT_BOX_X = 1000
    INPUT_BOX_Y = 601  # Moved up 4 pixels from 605
    INPUT_BOX_WIDTH = 210  # 1210 - 1000 (extends to 1210)
    INPUT_BOX_HEIGHT = 77  # Keep same height
    ...
    def __init__(self):
        ...
        # Log folders
        self.conversation_logs_dir = "lexi_conversation_logs"
        self.mood_logs_dir = "lexi_mood_logs"
        ...
        try:
            from modules.descriptor_analyzer import DescriptorAnalyzer
    ...
    def _get_date_stamped_filename(self, folder: str, prefix: str) -> str:
        """Get a date-stamped filename for today's log file."""
    ...
    def _init_log_files(self):
        ...
        try:
            ...
            conversation_log_file = self._get_date_stamped_filename(
                self.conversation_logs_dir, "lexi_conversation_log"
            )
            ...
            print(f"Warning: Could not initialize log files: {e}")
    
    def _log_message(self, role: str, content: str, descriptor: Optional[str] = None, action: Optional[str] = None):
        """Log a message to the date-stamped conversation log file with optional descriptor and action."""
        try:
    ...
    def _log_mood_history(self, emotion: str, action: Optional[str] = None):
        """Log mood/emotion history to the date-stamped mood log file."""
        try:
            timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            
            mood_log_file = self._get_date_stamped_filename(
                self.mood_logs_dir, "lexi_mood_history"
            )
            ...
            with open(mood_log_file, "a", encoding="utf-8") as f:
                ...
                if action:
                    parts.append(f"[{action}]")
                mood_content = " ".join(parts)
                f.write(f"[{timestamp}] {mood_content}\n")
        except Exception as e:
            print(f"Warning: Could not write to mood log file: {e}")
        ...
        def stream_thread():
            try:
                ...
                if response_to_save and not response_to_save.startswith("[ERROR"):
                    ...
                    # Log mood history
                    self._log_mood_history(emotion, action)
            except Exception as e:
                error_msg = f"[ERROR: {e}]"
